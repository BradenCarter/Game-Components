...
public static class UIHelpers
{
    ...
    //Exmaple: MyScrollRect.content.localPosition = MyScrollRect.GetSnapToPositionToBringChildIntoView(someChild);
    ...
    public static class UIListBuilder
    {
        ...
        public static void UpdateDisplays<T, U>(ICollection<T> itemList, GameObject template, Transform container, ref List<U> displays, Action<U, int> OnDisplaySet = null) where U : MonoBehaviour
        {
            for (int i = 0; i < itemList.Count; i++)
            {
                if (displays.Count <= i)
                {
                    displays.Add(UnityEngine.Object.Instantiate<GameObject>(template.gameObject, container).GetComponent<U>());
                }
                if (OnDisplaySet != null)
                    OnDisplaySet(displays[i], i);
